...
Files_::Files_(const std::string& filespec)
...
bool Files_::open(direction dirn, type typ)
{
  dirn_ = dirn;
  typ_ = typ;
  if(dirn == in)
...
void Files_::putLine(const std::string& s, bool wantReturn)
{
...
Block Files_::getBlock(size_t size)
{
  if(pIStream == 0 || !pIStream->good())
    throw std::runtime_error("input stream not open");
  if(typ_ != binary)
...
void Files_::putBlock(const Block& blk)
{
  if(pOStream == 0 || !pOStream->good())
    throw std::runtime_error("output stream not open");
  if(typ_ != binary)
...
/////////////////
/* file exists */
bool Files_::exists(const std::string& file)
{
...
///////////////
/* copy file */
bool Files_::copy(const std::string& src, const std::string& dst, bool failIfExists)
{
...
/////////////////
/* remove file */
bool Files_::remove(const std::string& file)
{
...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  Files_ me("../debug/filesystemdemo.exe");
  me.open(Files_::in, Files_::binary);
  ...
  {
	  Files_ you("c:/temp/fileSystemdemo.exe");
    you.open(Files_::out, Files_::binary);
    if(you.isGood())
    {
      while(me.isGood())
      {
