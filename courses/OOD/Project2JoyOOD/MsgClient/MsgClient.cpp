...
bool MsgClient::sendFile(const std::string& filename, Socket& socket)
{
	...
	file.open(FileSystem::Files_::in, FileSystem::Files_::binary);
	if (!file.isGood())
		return false;
...
void MsgClient::execute(const size_t TimeBetweenMessages, const size_t NumMessages)
{
	...
	Show::start();
	Show::title("Starting HttpMessage client" + myCountString + " on thread " + Utilities::Converter<std::thread::id>::toString(std::this_thread::get_id()));
	try {
		SocketSystem ss;
		SocketConnecter si;
		...
		for (size_t i = 0; i < NumMessages; ++i) {
			std::string msgBody = "<msg>Message #" + Converter<size_t>::toString(i + 1) + " from client #" + myCountString + "</msg>";
			msg = makeMessage(1, msgBody, "localhost:8080");
			/* Sender class will need to accept messages from an input queue and examine the toAddr attribute to see if a new connection
			is needed.  If so, it would either close the existing connection or save it in a map[url] = socket, then open a new connection.*/
			sendMessage(msg, si);
			Show::write("\n\n  client" + myCountString + " sent\n" + msg.toIndentedString());
			::Sleep(TimeBetweenMessages);
		}
		//  send all *.cpp and *.h files from TestFiles folder
		std::vector<std::string> filesCpp = FileSystem::Directory::getFiles("../TestFiles", "*.cpp");
		for (size_t i = 0; i < filesCpp.size(); ++i) {
			Show::write("\n\n  sending file " + filesCpp[i]);
			sendFile(filesCpp[i], si);
		}
		std::vector<std::string> filesH = FileSystem::Directory::getFiles("../TestFiles", "*.h");
		for (size_t i = 0; i < filesH.size(); ++i) {
			Show::write("\n\n  sending file " + filesH[i]);
			sendFile(filesH[i], si);
		}
		// shut down server's client handler
		msg = makeMessage(1, "quit", "toAddr:localhost:8080");
		sendMessage(msg, si);
		Show::write("\n\n  client" + myCountString + " sent\n" + msg.toIndentedString()); Show::write("\n"); Show::write("\n  All done folks");
	}
